...
resource "null_resource" "cluster_bootstrap" {
  ...
  provisioner "local-exec" {
    command = "bash ../../../scripts/validate-kustomize.sh && bash ../../../scripts/setup_cluster.sh ${module.eks.cluster_name} ${var.aws_region} ${aws_ecr_repository.my_app.repository_url} && bash ../../../scripts/deploy-argocd-apps.sh ${module.eks.cluster_name} ${var.aws_region} ${aws_ecr_repository.my_app.repository_url} > setup.log 2>&1"
